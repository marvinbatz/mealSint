...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["idgenero_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["genero_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["genero_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["idgenero_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idgenero_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_idgenero_' + iSeqRow).bind('blur', function() { sc_form_genero_idgenero__onblur(this, iSeqRow) })
                                       .bind('change', function() { sc_form_genero_idgenero__onchange(this, iSeqRow) })
                                       .bind('focus', function() { sc_form_genero_idgenero__onfocus(this, iSeqRow) });
  $('#id_sc_field_genero_' + iSeqRow).bind('blur', function() { sc_form_genero_genero__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_genero_genero__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_genero_genero__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_genero_idgenero__onblur(oThis, iSeqRow) {
  do_ajax_form_genero_validate_idgenero_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_genero_idgenero__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_genero_idgenero__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_genero_genero__onblur(oThis, iSeqRow) {
  do_ajax_form_genero_validate_genero_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_genero_genero__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_genero_genero__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_row(row, status) {
	...
	displayChange_field_idgenero_(row, status);
}
...
function displayChange_field_idgenero_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_genero_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_genero')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
