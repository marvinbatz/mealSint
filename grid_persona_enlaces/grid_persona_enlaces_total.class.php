...
class grid_persona_enlaces_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_persona_enlaces']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_persona_enlaces']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['tot_geral'] = array() ;  
      $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['contr_total_geral'] = "OK";
   } 

   //-----  pa_nombre
   function quebra_pa_nombre_sc_free_group_by($pa_nombre, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_pa_nombre = array() ;  
      $tot_pa_nombre[0] = $pa_nombre ; 
   }
   //-----  de_nombre
   function quebra_de_nombre_sc_free_group_by($de_nombre, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_de_nombre = array() ;  
      $tot_de_nombre[0] = $de_nombre ; 
   }
   //-----  mu_nombre
   function quebra_mu_nombre_sc_free_group_by($mu_nombre, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_mu_nombre = array() ;  
      $tot_mu_nombre[0] = $mu_nombre ; 
   }
   //-----  getario
   ...
   //-----  g_genero
   function quebra_g_genero_sc_free_group_by($g_genero, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_g_genero = array() ;  
      $tot_g_genero[0] = $g_genero ; 
   }

   //----- 
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang, $qrcode;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('pa_nombre' => "pa.nombre",'de_nombre' => "de.nombre",'mu_nombre' => "mu.nombre",'getario' => "IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>59,'60 años o más',IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>49,'50 a 59 años',IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>17,'18 a 49 años',IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>4,'5 a 17 años',IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>2,'25 a 59 meses','0 a 24 meses')))))",'g_genero' => "g.genero");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('p_idpersona' => 'N','anios' => 'N','meses' => 'N');
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_enlaces']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['pa_nombre']['reg'] = "S";
      $format_dimensions['pa_nombre']['msk'] = "";
      $format_dimensions['de_nombre']['reg'] = "S";
      $format_dimensions['de_nombre']['msk'] = "";
      $format_dimensions['mu_nombre']['reg'] = "S";
      $format_dimensions['mu_nombre']['msk'] = "";
      $format_dimensions['getario']['reg'] = "S";
      $format_dimensions['getario']['msk'] = "";
      $format_dimensions['g_genero']['reg'] = "S";
      $format_dimensions['g_genero']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "pa_nombre") {
              }
              if ($Cada_dim == "de_nombre") {
              }
              if ($Cada_dim == "mu_nombre") {
              }
              if ($Cada_dim == "getario") {
              }
              if ($Cada_dim == "g_genero") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
