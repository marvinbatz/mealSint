...
$old_error_handler = set_error_handler("seg_Login_scriptcase_error_handler");

function seg_Login_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['seg_Login']['contr_erro']) && $_SESSION['scriptcase']['seg_Login']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_seg_Login->contr_seg_Login->Apl_com_erro = true;
            if ($inicial_seg_Login->contr_seg_Login->NM_ajax_flag)
            {
                $inicial_seg_Login->contr_seg_Login->NM_ajax_info['errList']['geral_seg_Login'][] = $msg;
                return;
            }
            seg_Login_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function seg_Login_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['seg_Login'])  && '' != $_SESSION['scriptcase']['error_icon']['seg_Login'])  ? $_SESSION['scriptcase']['error_icon']['seg_Login']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['seg_Login']) && '' != $_SESSION['scriptcase']['error_close']['seg_Login']) ? $_SESSION['scriptcase']['error_close']['seg_Login'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['seg_Login']) && '' != $_SESSION['scriptcase']['error_span_title']['seg_Login']) ? $_SESSION['scriptcase']['error_span_title']['seg_Login'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['seg_Login']) && '' != $_SESSION['scriptcase']['error_icon_title']['seg_Login']) ? $_SESSION['scriptcase']['error_icon_title']['seg_Login'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['seg_Login']['error_buffer'] .= ob_get_flush();
}
