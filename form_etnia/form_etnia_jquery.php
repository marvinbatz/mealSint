...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["idetnia_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["idetnia_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idetnia_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_idetnia_' + iSeqRow).bind('blur', function() { sc_form_etnia_idetnia__onblur(this, iSeqRow) })
                                      .bind('change', function() { sc_form_etnia_idetnia__onchange(this, iSeqRow) })
                                      .bind('focus', function() { sc_form_etnia_idetnia__onfocus(this, iSeqRow) });
  $('#id_sc_field_nombre_' + iSeqRow).bind('blur', function() { sc_form_etnia_nombre__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_etnia_nombre__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_etnia_nombre__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_etnia_idetnia__onblur(oThis, iSeqRow) {
  do_ajax_form_etnia_validate_idetnia_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_etnia_idetnia__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_etnia_idetnia__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_etnia_nombre__onblur(oThis, iSeqRow) {
  do_ajax_form_etnia_validate_nombre_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_etnia_nombre__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_etnia_nombre__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_row(row, status) {
	...
	displayChange_field_idetnia_(row, status);
}
...
function displayChange_field_idetnia_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_etnia_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_etnia')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
