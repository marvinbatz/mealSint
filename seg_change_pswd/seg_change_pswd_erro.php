...
$old_error_handler = set_error_handler("seg_change_pswd_scriptcase_error_handler");

function seg_change_pswd_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['seg_change_pswd']['contr_erro']) && $_SESSION['scriptcase']['seg_change_pswd']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_seg_change_pswd->contr_seg_change_pswd->Apl_com_erro = true;
            if ($inicial_seg_change_pswd->contr_seg_change_pswd->NM_ajax_flag)
            {
                $inicial_seg_change_pswd->contr_seg_change_pswd->NM_ajax_info['errList']['geral_seg_change_pswd'][] = $msg;
                return;
            }
            seg_change_pswd_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function seg_change_pswd_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['seg_change_pswd'])  && '' != $_SESSION['scriptcase']['error_icon']['seg_change_pswd'])  ? $_SESSION['scriptcase']['error_icon']['seg_change_pswd']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['seg_change_pswd']) && '' != $_SESSION['scriptcase']['error_close']['seg_change_pswd']) ? $_SESSION['scriptcase']['error_close']['seg_change_pswd'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['seg_change_pswd']) && '' != $_SESSION['scriptcase']['error_span_title']['seg_change_pswd']) ? $_SESSION['scriptcase']['error_span_title']['seg_change_pswd'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['seg_change_pswd']) && '' != $_SESSION['scriptcase']['error_icon_title']['seg_change_pswd']) ? $_SESSION['scriptcase']['error_icon_title']['seg_change_pswd'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['seg_change_pswd']['error_buffer'] .= ob_get_flush();
}
