...
class grid_persona_lista_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_persona_lista']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_persona_lista']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $p_iddepartamento, $p_idmunicipio;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['tot_geral'] = array() ;  
      $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['contr_total_geral'] = "OK";
   } 

   //-----  p_iddepartamento
   function quebra_p_iddepartamento_sc_free_group_by($p_iddepartamento, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $p_iddepartamento, $p_idmunicipio;
      $tot_p_iddepartamento = array() ;  
      $tot_p_iddepartamento[0] = $p_iddepartamento ; 
   }
   //-----  p_idmunicipio
   function quebra_p_idmunicipio_sc_free_group_by($p_idmunicipio, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $p_iddepartamento, $p_idmunicipio;
      $tot_p_idmunicipio = array() ;  
      $tot_p_idmunicipio[0] = $p_idmunicipio ; 
   }
   //-----  getario
   function quebra_getario_sc_free_group_by($getario, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $p_iddepartamento, $p_idmunicipio;
      $tot_getario = array() ;  
      $tot_getario[0] = $getario ; 
   }

   //----- 
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang, $firma, $carnet, $qrcode, $codbar, $p_iddepartamento, $p_idmunicipio;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('p_iddepartamento' => "p.idDepartamento",'p_idmunicipio' => "p.idMunicipio",'getario' => "IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>59,'60 años o más',IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>49,'50 a 59 años',IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>17,'18 a 49 años',IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>4,'5 a 17 años',IF(YEAR( CURDATE( ) ) - YEAR( fechaDeNacimiento ) - IF( MONTH( CURDATE( ) ) < MONTH( fechaDeNacimiento),1,IF ( MONTH(CURDATE( )) = MONTH(fechaDeNacimiento),IF (DAY( CURDATE( ) ) < DAY( fechaDeNacimiento ),1,0 ),0))>2,'25 a 59 meses','0 a 24 meses')))))");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('p_idpersona' => 'N','anios' => 'N','meses' => 'N','p_iddepartamento' => 'N','p_idmunicipio' => 'N');
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_persona_lista']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['p_iddepartamento']['reg'] = "S";
      $format_dimensions['p_iddepartamento']['msk'] = "";
      $format_dimensions['p_idmunicipio']['reg'] = "S";
      $format_dimensions['p_idmunicipio']['msk'] = "";
      $format_dimensions['getario']['reg'] = "S";
      $format_dimensions['getario']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "p_iddepartamento") {
                  $this->Lookup->lookup_sc_free_group_by_p_iddepartamento($$Cada_dim,  $p_iddepartamento);
              }
              if ($Cada_dim == "p_idmunicipio") {
                  $this->Lookup->lookup_sc_free_group_by_p_idmunicipio($$Cada_dim,  $p_idmunicipio);
              }
              if ($Cada_dim == "getario") {
              }
              if (null === $$Cada_dim)
