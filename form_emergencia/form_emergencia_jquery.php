...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["idemergencia_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["idemergencia_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idemergencia_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_idemergencia_' + iSeqRow).bind('blur', function() { sc_form_emergencia_idemergencia__onblur(this, iSeqRow) })
                                           .bind('change', function() { sc_form_emergencia_idemergencia__onchange(this, iSeqRow) })
                                           .bind('focus', function() { sc_form_emergencia_idemergencia__onfocus(this, iSeqRow) });
  $('#id_sc_field_nombre_' + iSeqRow).bind('blur', function() { sc_form_emergencia_nombre__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_emergencia_nombre__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_emergencia_nombre__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_emergencia_idemergencia__onblur(oThis, iSeqRow) {
  do_ajax_form_emergencia_validate_idemergencia_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_emergencia_idemergencia__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_emergencia_idemergencia__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_emergencia_nombre__onblur(oThis, iSeqRow) {
  do_ajax_form_emergencia_validate_nombre_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_emergencia_nombre__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_emergencia_nombre__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_row(row, status) {
	...
	displayChange_field_idemergencia_(row, status);
}
...
function displayChange_field_idemergencia_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_emergencia_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_emergencia')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
