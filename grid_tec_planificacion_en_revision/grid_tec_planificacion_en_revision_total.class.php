...
class grid_tec_planificacion_en_revision_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_tec_planificacion_en_revision']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_tec_planificacion_en_revision']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $categoria;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['tot_geral'] = array() ;  
      $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      while (!$rt->EOF) 
      {
         if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['tot_geral'][1])) 
         {
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['tot_geral'][1] = 1 ; 
         } 
         else
         {
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['tot_geral'][1] += 1 ; 
      ...
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['contr_total_geral'] = "OK";
   } 
   ...
   {
      global $nm_lang, $categoria;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('pl_idproyecto' => "pl.idProyecto");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('pl_idplanificacion' => 'N','categoria' => 'N','pl_idtipobeneficiario' => 'N','pl_idproyecto' => 'N','pl_meta' => 'N','pl_conteo' => 'N','avance' => 'N');
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tec_planificacion_en_revision']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
