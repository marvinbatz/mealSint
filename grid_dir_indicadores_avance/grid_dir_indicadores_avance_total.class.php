...
class grid_dir_indicadores_avance_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_dir_indicadores_avance']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_dir_indicadores_avance']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $planmeal_idobjetivo_resultado, $planmeal_idunidaddemedida;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['tot_geral'] = array() ;  
      $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['contr_total_geral'] = "OK";
   } 

   //-----  proyecto_idproyecto
   function quebra_proyecto_idproyecto_sc_free_group_by($proyecto_idproyecto, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $planmeal_idobjetivo_resultado, $planmeal_idunidaddemedida;
      $tot_proyecto_idproyecto = array() ;  
      $tot_proyecto_idproyecto[0] = $proyecto_idproyecto ; 
   }

   //----- 
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang, $planmeal_idobjetivo_resultado, $planmeal_idunidaddemedida;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('proyecto_idproyecto' => "proyecto.idProyecto");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('planmeal_idplanmeal' => 'N','proyecto_idproyecto' => 'N','planmeal_idobjetivo_resultado' => 'N','planmeal_idunidaddemedida' => 'N');
     ...
     $comando .= " ON " . $join;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_dir_indicadores_avance']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['proyecto_idproyecto']['reg'] = "S";
      $format_dimensions['proyecto_idproyecto']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "proyecto_idproyecto") {
                  $this->Lookup->lookup_sc_free_group_by_proyecto_idproyecto($$Cada_dim,  $proyecto_idproyecto);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
