...
*/
    ...
    session_start();
    $_SESSION['scriptcase']['grid_jdp_planificacion_institucion_act']['glo_nm_path_imag_temp']  = "/scriptcase/tmp";
    //check tmp
    ...
    $SC_cod_proj = "MealSint2";
    $SC_apl_proc = "grid_jdp_planificacion_institucion_act";
    $SC_conf_opt = explode(",","tem_res_cons,tem_res_res,nm_json_label,json_format");
/* sc_apl_default */
...
<?php
    ...
    $json_format = (isset($_GET['nm_json_format'])) ? $_GET['nm_json_format'] : "S";
    $json_label  = (isset($_GET['nm_json_label']))  ? $_GET['nm_json_label']  : "N";
    ...
    $ini_json_res = explode(",", $_GET['nm_ini_json_res']);
...
*/
    ...
    $tradutor = array();
    if (isset($_SESSION['scriptcase']['sc_idioma_json']))
    {
        $tradutor = $_SESSION['scriptcase']['sc_idioma_json'];
    }
...
<form name="config_json" method="post" action="" autocomplete="off">
<?php
$pos = "left";
 ...
 <td align="center">
  ...
  <div id="tabs">
    ...
    <div id="tabs-general" style="padding: 0px; margin: 0px">
      ...
      {
          $Opt_display = (($origem == 'cons' && !in_array("tem_res_res", $SC_conf_opt)) || ($origem == 'res' && !in_array("tem_res_cons", $SC_conf_opt))) ? ' style="display: none"' : '';
      ?>
...
?>
          ...
          <?php
             foreach ($all_modules as $cada_mod)
             {
                 if ($cada_mod != 'chart')
                 {
                     $ckeck = (in_array($cada_mod, $ini_json_res)) ? "checked" : "";
                 ?>
                    <label>
      ...
      <?php
        ...
        $Opt_display = (!in_array("json_format", $SC_conf_opt)) ? ' style="display: none"' : '';
      ?>
         <tr<?php echo $Opt_display ?>>
           <td nowrap class="scGridFieldOddFont" align="left">
...
?>
       ...
       <?php
        ...
        ?>
             <label><input type=radio name="nm_json_format" value="S" <?php echo $ckeck ?>> <?php echo $tradutor[$language]['json_yes']; ?></label>
        <?php
           $ckeck = ($json_format == "N") ? "checked" : "";
        ?>
      ...
      <?php
        $Opt_display = (!in_array("nm_json_label", $SC_conf_opt)) ? ' style="display: none"' : '';
      ?>
         <tr<?php echo $Opt_display ?>>
           <td nowrap class="scGridFieldOddFont" align="left">
...
?>
       ...
       <?php
        ...
        ?>
             <label><input type=radio name="nm_json_label" value="S" <?php echo $ckeck ?>> <?php echo $tradutor[$language]['json_yes']; ?></label>
        <?php
           $ckeck = ($json_label == "N") ? "checked" : "";
        ?>
...
<?php
  ...
  {
?>
   document.config_json.password.value = "";
<?php
  }
?>
});
  ...
  function processa()
  {
     ...
     json_format = document.config_json.nm_json_format.value;
     json_label  = document.config_json.nm_json_label.value;

     res_cons = "";
     if (document.config_json.id_tem_res_cons)
     {
     ...
     {
      ?>
         use_pass = document.config_json.password.value;
      <?php
     }
     ?>
...
<?php
     if ($export_ajax == 'S') {
?>
         parent.nm_gp_json_conf(json_format, json_label, res_cons, use_pass, '<?php echo NM_encode_input($export_ajax); ?>', 'json', false);return false;
<?php
     }
     else if ($export_ajax == 'R') {
?>
         parent.nm_gp_json_conf(json_format, json_label, res_cons, use_pass, '<?php echo NM_encode_input($export_ajax); ?>', 'json_res', false);return false;
<?php
     } else {
?>
         parent.nm_gp_json_conf(json_format, json_label, res_cons, use_pass, '<?php echo NM_encode_input($export_ajax); ?>', '', false);return false;
<?php
     }
?>
/*--------*/
