  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_seg_sync_apps_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_seg_sync_apps_mob_form();
	}
...
?>
  ...
  // ---------- Validate sync
  function do_ajax_seg_sync_apps_mob_validate_sync()
  {
  } // do_ajax_seg_sync_apps_mob_validate_sync

  function do_ajax_seg_sync_apps_mob_validate_sync_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_seg_sync_apps_mob_validate_sync_cb

  // ---------- Validate check_deleted
  function do_ajax_seg_sync_apps_mob_validate_check_deleted()
  {
    var nomeCampo_check_deleted = "check_deleted";
    var var_check_deleted = scAjaxGetFieldCheckbox(nomeCampo_check_deleted, ";");
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_seg_sync_apps_mob_validate_check_deleted(var_check_deleted, var_script_case_init, do_ajax_seg_sync_apps_mob_validate_check_deleted_cb);
  } // do_ajax_seg_sync_apps_mob_validate_check_deleted

  function do_ajax_seg_sync_apps_mob_validate_check_deleted_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "check_deleted";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sAppErrors = scAjaxListErrors(true);
    if ("" == sAppErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_seg_sync_apps_mob_validate_check_deleted_cb
...
function scJs_confirm(message, callbackOk, callbackCancel) {
  ...
  // ---------- Form
  function do_ajax_seg_sync_apps_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_seg_sync_apps_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_sync = scAjaxGetFieldHidden("sync");
    var var_check_deleted = scAjaxGetFieldCheckbox("check_deleted", ";");
    ...
    scAjaxProcOn();
    x_ajax_seg_sync_apps_mob_submit_form(var_sync, var_check_deleted, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_seg_sync_apps_mob_submit_form_cb);
  } // do_ajax_seg_sync_apps_mob_submit_form

  function do_ajax_seg_sync_apps_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_seg_sync_apps_mob_submit_form_cb_after_alert);
  } // do_ajax_seg_sync_apps_mob_submit_form_cb
  function do_ajax_seg_sync_apps_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_seg_sync_apps_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_seg_sync_apps_mob_navigate_form()
  {
    perform_ajax_seg_sync_apps_mob_navigate_form();
  }

  function perform_ajax_seg_sync_apps_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_seg_sync_apps_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['seg_sync_apps_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_seg_sync_apps_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_seg_sync_apps_mob_navigate_form_cb_after_alert);
  } // do_ajax_seg_sync_apps_mob_navigate_form_cb
  function do_ajax_seg_sync_apps_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_seg_sync_apps_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_seg_sync_apps_mob_navigate_form_cb_after_alert
  function sc_hide_seg_sync_apps_mob_form()
  {
  ...
  ajax_field_list[0] = "sync";
  ajax_field_list[1] = "check_deleted";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "sync": {"label": "<?php echo $this->Ini->Nm_lang['lang_sync_apps'] ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
    "check_deleted": {"label": "", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  var ajax_field_mult = {
    "sync": new Array(),
    "check_deleted": new Array()
  };
  ajax_field_mult["sync"][1] = "sync";
  ajax_field_mult["check_deleted"][1] = "check_deleted";

  var ajax_field_id = {
    "sync": new Array("hidden_field_label_sync", "hidden_field_data_sync"),
    "check_deleted": new Array("hidden_field_label_check_deleted", "hidden_field_data_check_deleted")
  };
