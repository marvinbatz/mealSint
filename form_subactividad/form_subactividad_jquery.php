...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["idsubactividad_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["idsubactividad_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idsubactividad_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scEventControl_onFocus(oField, iSeq) {
  ...
  if ("idactividad_" + iSeq == fieldName) {
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_idsubactividad_' + iSeqRow).bind('blur', function() { sc_form_subactividad_idsubactividad__onblur(this, iSeqRow) })
                                             .bind('change', function() { sc_form_subactividad_idsubactividad__onchange(this, iSeqRow) })
                                             .bind('focus', function() { sc_form_subactividad_idsubactividad__onfocus(this, iSeqRow) });
  $('#id_sc_field_nombre_' + iSeqRow).bind('blur', function() { sc_form_subactividad_nombre__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_subactividad_nombre__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_subactividad_nombre__onfocus(this, iSeqRow) });
  $('#id_sc_field_descripcion_' + iSeqRow).bind('blur', function() { sc_form_subactividad_descripcion__onblur(this, iSeqRow) })
                                          .bind('change', function() { sc_form_subactividad_descripcion__onchange(this, iSeqRow) })
                                          .bind('focus', function() { sc_form_subactividad_descripcion__onfocus(this, iSeqRow) });
  $('#id_sc_field_codigo_' + iSeqRow).bind('blur', function() { sc_form_subactividad_codigo__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_subactividad_codigo__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_subactividad_codigo__onfocus(this, iSeqRow) });
  $('#id_sc_field_idactividad_' + iSeqRow).bind('change', function() { sc_form_subactividad_idactividad__onchange(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_subactividad_idsubactividad__onblur(oThis, iSeqRow) {
  do_ajax_form_subactividad_validate_idsubactividad_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_subactividad_idsubactividad__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_subactividad_idsubactividad__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_subactividad_nombre__onblur(oThis, iSeqRow) {
  do_ajax_form_subactividad_validate_nombre_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_subactividad_nombre__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_subactividad_nombre__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_subactividad_descripcion__onblur(oThis, iSeqRow) {
  do_ajax_form_subactividad_validate_descripcion_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_subactividad_descripcion__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_subactividad_descripcion__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_subactividad_codigo__onblur(oThis, iSeqRow) {
  do_ajax_form_subactividad_validate_codigo_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_subactividad_codigo__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_subactividad_codigo__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_subactividad_idactividad__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}
...
function displayChange_row(row, status) {
	...
	displayChange_field_idsubactividad_(row, status);
}
...
function displayChange_field_idsubactividad_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_subactividad_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_subactividad')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
...
function scJQSelect2Add(seqRow, specificField) {
  if (null == specificField || "idactividad_" == specificField) {
    scJQSelect2Add_idactividad_(seqRow);
  }
} // scJQSelect2Add

function scJQSelect2Add_idactividad_(seqRow) {
  var elemSelector = "all" == seqRow ? ".css_idactividad__obj" : "#id_sc_field_idactividad_" + seqRow;
  $(elemSelector).select2(
    {
      containerCssClass: 'css_idactividad__obj',
      dropdownCssClass: 'css_idactividad__obj',
      language: {
        noResults: function() {
...
function scJQElementsAdd(iLine) {
  ...
  setTimeout(function () { if ('function' == typeof displayChange_field_idactividad_) { displayChange_field_idactividad_(iLine, "on"); } }, 150);
} // scJQElementsAdd
